...
describe('claim prototype safeguards and attribution UI', () => {
  ...
  it('summarizes benefits the signed-in user recorded by recipient, not all-prototype totals', () => {
    ...
    expect(claimSource).toContain('Yourself')
    expect(claimSource).toContain('Dependents')
    expect(claimSource).toContain('On behalf of others')
    expect(claimSource).toContain('Benefits you recorded for yourself')
    expect(claimSource).toContain('Benefits you recorded for your dependents')
    expect(claimSource).toContain('Attributed to those people, not to you')
    expect(claimSource).toContain('onBehalfOfOthers')
    expect(claimSource).toContain('claimed · ')
    expect(claimSource).toContain('on behalf of other people')
    expect(claimSource).toContain('everyone else’s prototype totals')
    ...
    expect(claimSource).not.toContain('Benefits you recorded for others')
  })
})
